...
public class ViewActivitySupp extends Activity implements OnClickListener, OnItemSelectedListener {
	...
	private Spinner spp;
	private TextView title, name, people, types, stime, etime, maxcon;
	...
	private void findViewById() {
		...
		title = (TextView) findViewById(R.id.supp_title);
	...
	private void edtSpp() {
		if (sps.equals("缺人")) {
			...
			String sf = ms.substring(0, 1) + MainActivity.getLine(sps, 0, '(') + ms.substring(1);
		...
		if (ps < MainActivity.Worklist.size()) {
			...
			for (int i = 0; i < MainActivity.Daylist.size(); i++) {
				...
				case "2":
					...
					String au = pm + "(值班)";
					if (!an.equals(pm) && !"99".equals(pm)) {
						boolean add = true;
						for (int us = 0; us < userwork.size(); us++) {
							if (userwork.get(us).equals(au)) {
								add = false;
							}
						}
						if (add) {
