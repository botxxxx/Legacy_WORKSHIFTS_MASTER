...
public class MainActivity extends FragmentActivity
	...
	private static boolean link = false, update = false, net = false;
	private static int CY, CM, page, pos, uitem = 0;
	private static String item, name = "MASTER";
	...
	public static boolean debug = false, linker;
	public static int Years, Months, Days, about;
	public static String email = "", ver = "未更新", GU, GW, GM;
	...
	private void load() {
		...
		if (list.size() > 0) {
			ver = list.get(0);
			CY = Integer.parseInt(list.get(1));
			CM = Integer.parseInt(list.get(2));
			if (list.size() == 4) {
				email = list.get(3);
			}
			Years = CY;
	...
	private void setView() {
		...
		// im_menu.setOnLongClickListener(this);
	...
	private void setHandShake() {
		if (net && link) {
			...
			new Timer(true).schedule(new TimerTask() {
				public void run() {
					if (!linker) {
						UnLinkHandler.obtainMessage().sendToTarget();
					} else {
						LinkHandler.obtainMessage().sendToTarget();
					}
				}
			}, 3000);
		}
	...
	public static String getWork(String c) {
		// "11111"
		int r = 0;
		ArrayList<String> list = new ArrayList<String>();
		for (int i = 0; i < 5; i++) {
	...
	public static void edtRecovey(Context con) {
		for (int day = 0; day < Datelist.size(); day++) {
			out("g5/" + getSQLdate() + getSQLD(day) + '/' + Datelist.get(day));
		}
		toast(con, "已更新.");
	}
	...
	public static void addSppItem(Context con, String c) {
		...
		for (int a = 0; a < Daylist.size(); a++) {
			String sm = Daylist.get(a); // 00
			String st = sm.substring(0, 1); // 0:work,1:user
			if ("2".equals(st)) {
				String ss = sm.substring(1);
				String pm = getLine(ss, 0, '/');
				if (c.substring(1).equals(pm)) {
					p++;
				}
			}
		}
	...
	public static String getSQLD(int D) {
		if (D < 10) {
			return "0" + D;
		} else {
			return "" + D;
	...
	public static boolean getFinal(String m) {
		String ms = Datelist.get(0);
		int r = 0;
		String sr = getLine(ms, r, '/');
		r += sr.length() + 1;
	...
	public static void day_upload(Context con) {
		...
		if (uitem != 0 && ((uitem - 1) < Daylist.size())) {
			Daylistview.setSelection(uitem - 1);
		}
	}
				...
				for (int i = 1; i < Datelist.size(); i++) {
					...
					for (int di = 0; di < dlist.size(); di++) {
						if (dlist.get(di).substring(0, 1).equals("2")) {
							if (getLine(dlist.get(di), 1, '/').equals("99")) {
	...
	private Handler UnLinkHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			link = false;
